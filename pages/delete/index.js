...
function Delete() {
  return (
    <>
      ...
      {/* <div>
        {" "}
        // COMPONENTS // COMPONENTS // ---------- // Main // UI //
        ButtonCategories // ProductItems // ProductItem // Components in reverse
        order of above const ProductItem = ({(category, name)}) => (
        ...
        {super(props)}; this.state = {displayCategory}: "all", products:
        props.products, productCategories: props.productCategories, };
        this.setCategory = this.setCategory.bind(this); } setCategory(category){" "}
        {this.setState({
          displayCategory: category,
        })}
        ; } render() {}
        return <UI setCategory={this.setCategory} state={this.state} />; } } //
        data const PRODUCTS = [{category}: "entertainment", name: "Football" },
        ...
        {category}: "leisure", name: "Holiday" }, ]; // get unique category
        items const uniqueItems = (x, i, array) => array.indexOf(x) === i; const
        PRODUCT_CATEGORIES = PRODUCTS.map((prod) => prod.category).filter(
        uniqueItems ); PRODUCT_CATEGORIES.push("all");
        PRODUCT_CATEGORIES.sort(); ReactDOM.render(
